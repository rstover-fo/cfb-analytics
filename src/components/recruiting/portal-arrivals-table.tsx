...
import { ArrowUpDown, Star, LogIn, ChevronDown, ChevronUp, Calendar } from 'lucide-react';
import { cn } from '@/lib/utils';
import type { PortalArrival } from '@/lib/db/queries/transfers';

interface PortalArrivalsTableProps {
  data: PortalArrival[];
  year: number;
 ...
 */
export function PortalArrivalsTable({ data, year, initialLimit = 10 }: PortalArrivalsTableProps) {
  const [sortKey, setSortKey] = useState<SortKey>('rating');
  const [sortDirection, setSortDirection] = useState<SortDirection>('desc');
  const [expanded, setExpanded] = useState(false);

  if (data.length === 0) {
    return <PortalArrivalsTableEmpty year={year} />;
  }
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead
                  scope="col"
                  className="hover:bg-muted/50 hidden cursor-pointer xl:table-cell"
...
export function PortalArrivalsTableEmpty({ year }: { year?: number }) {
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-sm font-medium">
          <LogIn className="h-4 w-4" aria-hidden="true" />
          {year ? `${year} ` : ''}Portal Arrivals
        </CardTitle>
      </CardHeader>
      <CardContent>
        <p className="text-muted-foreground text-sm">
          No portal arrivals{year ? ` for ${year}` : ''}
        </p>
      </CardContent>
    </Card>
  );
...
export function PortalArrivalsTableSkeleton() {
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-sm font-medium">
      ...
      <CardContent>
        <div className="space-y-2">
          {/* Header skeleton */}
          <div className="grid grid-cols-5 gap-2 border-b pb-2 lg:grid-cols-6 xl:grid-cols-7">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="bg-muted h-4 animate-pulse rounded" />
            ))}
            <div className="bg-muted hidden h-4 animate-pulse rounded lg:block" />
            <div className="bg-muted hidden h-4 animate-pulse rounded xl:block" />
          </div>
          {/* Row skeletons */}
          {[...Array(10)].map((_, i) => (
            <div key={i} className="grid grid-cols-5 gap-2 py-2 lg:grid-cols-6 xl:grid-cols-7">
              {[...Array(5)].map((_, j) => (
                <div key={j} className="bg-muted h-5 animate-pulse rounded" />
              ))}
              <div className="bg-muted hidden h-5 animate-pulse rounded lg:block" />
              <div className="bg-muted hidden h-5 animate-pulse rounded xl:block" />
            </div>
          ))}
        </div>
      </CardContent>
